import { b as $$Image, v as ve, k as ke, $ as $$PageTitle, a as $$PagesLayout } from './_slug__xJ597Spt.mjs';
...
const $$PortfolioGrid = createComponent(async ($$result, $$props, $$slots) => {
  ...
  Astro2.props;
  return renderTemplate(_a || (_a = __template(["", '<ul class="flex flex-wrap w-full md:w-11/12 justify-start gap-2 md:gap-6 font-serif italic text-neutral-500 max-w-7xl mx-4 md:mx-auto mb-12 text-base leading-0"> ', ' </ul> <ul class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-12 md:gap-12 w-full md:w-11/12 max-w-7xl grid-flow-dense mx-auto"> ', " ", ' </ul> <script>\n  const filterPortfolioItems = (filtersArray, portfolioItems) => {\n    if (filtersArray.length === 0) {\n      portfolioItems.forEach((element) => {\n        element.classList.remove("hidden");\n      });\n    } else {\n      portfolioItems.forEach((element) => {\n        if (filtersArray.some((tag) => element.dataset.tags.includes(tag))) {\n          element.classList.remove("hidden");\n        } else {\n          element.classList.add("hidden");\n        }\n      });\n    }\n  };\n\n  const tagNames = document.querySelectorAll(\n    ".tag"\n  ) as NodeListOf<HTMLLIElement>;\n\n  const portfolioItems = document.querySelectorAll(\n    ".portfolio-item"\n  ) as NodeListOf<HTMLLIElement>;\n\n  let filterList: string[] = [];\n\n  tagNames.forEach((tag) => {\n    tag.addEventListener("click", (event) => {\n      if (tag.dataset.filter === "true") {\n        tag.dataset.filter = "false";\n        filterList = filterList.filter((filter) => filter !== tag.dataset.tag);\n      } else {\n        tag.dataset.filter = "true";\n        filterList.push(tag.dataset.tag);\n      }\n      filterPortfolioItems(filterList, portfolioItems);\n    });\n  });\n</script>'])), maybeRenderHead(), allTags.map((tag) => renderTemplate`<li> <span${addAttribute(tag, "data-tag")} data-filter="false" class=" tag hover:cursor-pointer hover:text-primary data-[filter=true]:font-bold"> ${tag} </span> <span>/</span> </li>`), featuredProject && renderTemplate`${renderComponent($$result, "PortfolioItem", $$PortfolioItem, { "title": featuredProject.title, "alt": featuredProject.image.alt, "image": featuredProject.image.filename, "featured": true, "slug": featuredProject.slug, "tags": featuredProject.tags })}`, projects.map((project) => renderTemplate`${renderComponent($$result, "PortfolioItem", $$PortfolioItem, { "title": project.title, "alt": project.image.alt, "image": project.image.filename, "slug": project.slug, "tags": project.tags, "featured": false })}`));
